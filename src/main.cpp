		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2') {
			...
			if (s != "") {
				...
				if (event == "telemetry") {
					...
					//If the previous path waypoint is size is almost empty, use the car as starting reference.
					if(prev_size<2){
					...
					for(int i=0; i<3; i++){
						...
						vector<double> next_waypoint = getXY(next_s, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);
						ptsx.push_back(next_waypoint.at(0));
						ptsy.push_back(next_waypoint.at(1));
					}
					...
					//Define the actual (x,y) point we will user for the planner
					...
					for(int i=0; i<= 50-previous_path_x.size(); i++){
						...
						//Convert the points back to global co-orindate system.
